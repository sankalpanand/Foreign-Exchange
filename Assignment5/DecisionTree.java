...
public class DecisionTree {
	...
	ArrayList<Character> finalDataLayout;
	 ...
	 */
	public DecisionTree(ArrayList<Character> finalDataLayout, ArrayList<ArrayList<String>> originalTrainingData, int M, int Ms) 
	{
		this.finalDataLayout = finalDataLayout;
		this.M = M;
		this.Ms = Ms;
		...
		BootStrapSample(originalTrainingData, sampledTrainingData, leftOutData);

		// Step 2- Generate tree from that sampled data
		root = CreateTree(sampledTrainingData);
		FlushData(root);
	}
	...
	private TreeNode CreateTree(ArrayList<ArrayList<String>> sampledTrainingData)
	{
		...
		if (!parent.isLeaf)
		{
			...
			for (int m : selectedFeatures)
			{
				...
				if (DataPointToCheck.size() > MIN_SIZE_TO_CHECK_EACH)
				{
					...
					{
						CheckPosition(m, DataPointToCheck.get(i), totalInstancesAtThisNode, lowestEntropy, parent);
				...
				else
				{
					...
					{
						CheckPosition(m, k, totalInstancesAtThisNode, lowestEntropy, parent);
	...
	{
		boolean[] whichVarsToInclude = new boolean[M];

		while(true)
		{
			// Keep generating random indexes until their count reaches Ms 
			int randomVal = (int) Math.floor(Math.random() * M);
		...
		ArrayList<Integer> shortRecord = new ArrayList<Integer>(Ms);
	...
	/** This method will take each test instance, takes it through entire training data to calculate prediction for it.
	 * @return A list of predictions */
	public ArrayList<String> runPredictions(ArrayList<ArrayList<String>> trainedInstances, ArrayList<ArrayList<String>> testInstances)
	{
		ArrayList<String> predictions = new ArrayList<String>();
		
		for(ArrayList<String> singleTestInstance : testInstances)
		{
			String result = Evaluate(singleTestInstance, trainedInstances);
	 ...
	 */
	private double CheckPosition(int splitOnThisFeature, int n, int totalInstancesAtThisNode, DoubleWrap lowestE, TreeNode parent)
	{
		...
		// CATEGORICAL
		if(finalDataLayout.get(splitOnThisFeature)=='C')
		{
